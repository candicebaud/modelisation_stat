...
predicted_values_hosp <- pred_hosp$fit[1:357]
predicted_values_rea <- pred_rea$fit[1:357]

w_hosp = (log(data_agreg$m_hosp) - predicted_values_hosp)/(predicted_values_hosp**alpha)
w_rea = (log(data_agreg$m_rea) - predicted_values_rea)/(predicted_values_rea**alpha)
...
for (i in 1:7){
  for (k in 0:(h-1)){
    s_hosp[i] = s_hosp[i] + w_hosp[i + 7*k]
    s_rea[i] = s_rea[i] + w_rea[i + 7*k]}
  s_hosp = s_hosp/h
  s_rea = s_rea/h
}
...
e_hat_hosp <- log(data_agreg$m_hosp) - predicted_values_hosp
w_hat_hosp <- e_hat_hosp / predicted_values_hosp
s_hat_hosp <- sapply(1:7, function(m) mean(w_hat_hosp[m + 7*(0:(h-1))]))
e_hat_hosp <- (e_hat_hosp - s_hat_hosp[rep(1:7, each=h)]) / predicted_values_hosp

checkresiduals(e_hat_hosp)
