...
De même ci-dessous pour les réanimations.

```{r, fig.align="center"}

g3 <- data_agreg %>% filter(jour < as.Date('2023-02-01', format = '%Y-%m-%d')) %>%ggplot(aes(x=jour,y=diff_log_rea_agreg))+
  geom_line() + xlab('jour') + ylab('Hospitalisations')
g3

g4 <- data_agreg %>% filter(jour < as.Date('2023-02-01', format = '%Y-%m-%d')) %>% group_by(week) %>% summarise(s=mean(diff_log_rea_agreg)) %>%ggplot(aes(x=week,y=s))+
  geom_line() + xlab('jour') + ylab('Hospitalisations')
...
autre méthode ? on retrouve pareil ...  
```{r, fig.align="center"}
data_agreg %>% mutate(
  diff_hosp = diff(Taux_agrégé_hosp)/Taux_agrégé_hosp
)%>% filter(jour < as.Date('2023-02-01', format = '%Y-%m-%d')) %>%ggplot(aes(x=jour,y=diff_hosp))+
  geom_line() + xlab('jour') + ylab('Hospitalisations')
